controllers
  ...
  .controller('RegisterCtrl', function ($scope, Upload, $http, Auth, sessionService, ionicDatePicker, $filter, $ionicLoading, $state, $rootScope, $window, $timeout, $ionicModal) {

    $ionicModal.fromTemplateUrl('templates/session/privacy.html', {
      scope: $scope,
    ...
    $scope.showPrivacy = function () {
      $scope.modal.show();
      //$scope.data = {}
      //$(".popup-container .popup").css({"width": "95% !important"})
      // Custom popup
      /* var privacy_popup = $ionicPopup.show({
           templateUrl: 'templates/session/privacy.html',
           title: 'ПОЛЬЗОВАТЕЛЬСКОЕ СОГЛАШЕНИЕ',
           scope: $scope,
           backdropClickToClose: true
       });

       IonicClosePopupService.register(privacy_popup);*/
    };
    ...
    $scope.createUser = function (valid, fileToUpload) {
      ...
      if (valid) {
        ...
        //$http.post('http://medappteka.uz/api/user/' + updateUrl, dataUpdate)
        /*$http({
            method: 'POST',
            url: 'http://medappteka.uz/api/user/' + updateUrl,
            data: dataUpdate,//Object.toparams($scope.signin),
            headers: {
                'Content-Type': false,
                'Process-Data' : false
            }
